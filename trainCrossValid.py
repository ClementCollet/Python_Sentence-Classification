...
tf.flags.DEFINE_string("dossier_sortie","./runs/Subjective/TestFullyConnected","Dossier avec le voc, les summaries ...")
...
tf.flags.DEFINE_integer("batch_size",100, "Taille de batch")
tf.flags.DEFINE_integer("nb_iteration",30, "Nombre de fois où on passe sur les données d'entrainement")
tf.flags.DEFINE_integer("evaluate_every",50, "Evaluation toutes les X étapes")
tf.flags.DEFINE_integer("10eme","","dixieme qui servira pour évaluation")
...
# Affiche les Parametres et les ecrit dans un fichier bilan
...
# Forcer une taille d'embedding de 300 si Word2Vec
if FLAGS.word2vec:
...
taille=len(y)
np.concatenate([positive_labels, negative_labels], 0)
dev_sample_index = int(FLAGS.percent_validation * float(len(y)))
x_val = x_shuffled[FLAGS.10eme*dev_sample_index+FLAGS.10eme:(FLAGS.10eme+1)*dev_sample_index+FLAGS.10eme+1]
y_val = y_shuffled[FLAGS.10eme*dev_sample_index+FLAGS.10eme:(FLAGS.10eme+1)*dev_sample_index+FLAGS.10eme+1]
if i=0:
    x_train = x_shuffled[(FLAGS.10eme+1):]
    y_train = y_shuffled[(FLAGS.10eme+1):]
if i=9:
    x_train = x_shuffled[:9*FLAGS.10eme+9]
    y_train = y_shuffled[:9*FLAGS.10eme+9]
else:
    x_train1, x_train2 = x_shuffled[:(i*FLAGS.10eme+i-1)],x_shuffled[((i+1)*FLAGS.10eme+i+2):]
    y_train1, y_train2 = y_shuffled[:(i*FLAGS.10eme+i-1)],y_shuffled[((i+1)*FLAGS.10eme+i+2):]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        # 3 Fichier, 1 avec toutes les precisions pour l'Entrainement, un autre pour l'évaluation, le dernier contient la precision a une même étape du dernier batch d'apprentisasge fournit et de la base d'évaluation
        ...
        # Chargement ou non d'un dictionnaire
        if FLAGS.word2vec:
